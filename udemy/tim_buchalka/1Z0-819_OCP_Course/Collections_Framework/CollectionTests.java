...
public class CollectionTests {
    ...
    // takes a collections of a given type, plus 3 variables of that type
    public static <T> void doStuffWithCollection(Collection<T> h, T addedValue1, T addedValue2, T removedValue) {
        String collectionType = h.getClass().getName();

        // Check if adding a null is valid or throws exception
        System.out.format("Collection type: [%s] - Initial State: %s\n", collectionType, h);
        try {
            System.out.format("Can null be added to %s? %s - New State: %s\n", collectionType, h.add(null), h);
        } catch (Exception e) {
        ...
        // Add an element
        System.out.println("Adding " + addedValue1 +
                " " + h.add(addedValue1) + " - new state:  " + h);
